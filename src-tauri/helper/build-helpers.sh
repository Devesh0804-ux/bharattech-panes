...
echo "Building BharatTechKeepAwakeHelper (universal)..."
...
lipo -create \
  ...
  -output "$OUTPUT_DIR/com.bharattech.app.helper.keepawake"

rm "$OUTPUT_DIR/keepawake-helper-arm64" "$OUTPUT_DIR/keepawake-helper-x86_64"

echo "Building BharatTechHelperRegistrar (universal)..."
swiftc -O \
  -target arm64-apple-macos${MIN_MACOS} \
...
lipo -create \
  ...
  -output "$OUTPUT_DIR/BharatTechHelperRegistrar"

rm "$OUTPUT_DIR/registrar-arm64" "$OUTPUT_DIR/registrar-x86_64"

echo "Helper binaries built in $OUTPUT_DIR"
ls -la "$OUTPUT_DIR/com.bharattech.app.helper.keepawake" "$OUTPUT_DIR/BharatTechHelperRegistrar"
