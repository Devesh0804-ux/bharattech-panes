// BharatTechKeepAwakeHelper — privileged daemon that toggles system-wide sleep
...
// Unix domain socket for JSON commands from the main BharatTech app.
//
...
private let socketPath = "/var/run/com.bharattech.app.keepawake.sock"
...
func createListenSocket() -> Int32 {
    ...
    guard fd >= 0 else {
        NSLog("BharatTechKeepAwakeHelper: failed to create socket: \(errno)")
        return -1
    }
    ...
    guard pathBytes.count <= MemoryLayout.size(ofValue: addr.sun_path) else {
        NSLog("BharatTechKeepAwakeHelper: socket path too long")
        close(fd)
        return -1
    }
    ...
    guard bindResult == 0 else {
        NSLog("BharatTechKeepAwakeHelper: failed to bind socket: \(errno)")
        close(fd)
        return -1
    }
    ...
    guard let _ = SCDynamicStoreCopyConsoleUser(nil, &consoleUid, &consoleGid) else {
        NSLog("BharatTechKeepAwakeHelper: no console user found, refusing to create socket")
        close(fd)
        unlink(socketPath)
        return -1
    }
    ...
    guard listen(fd, 2) == 0 else {
        NSLog("BharatTechKeepAwakeHelper: failed to listen on socket: \(errno)")
        close(fd)
        return -1
    }
    ...
    if getpeereid(clientFd, &peerUid, &peerGid) == 0 {
        ...
        if let _ = SCDynamicStoreCopyConsoleUser(nil, &consoleUid, nil),
           peerUid != consoleUid && peerUid != 0 {
            NSLog("BharatTechKeepAwakeHelper: rejected connection from uid \(peerUid) (console uid \(consoleUid))")
            return
        }
    }
...
func installSignalHandlers() {
    signal(SIGTERM) { _ in
        NSLog("BharatTechKeepAwakeHelper: received SIGTERM, restoring sleep")
        _ = allowSleep()
    ...
    signal(SIGINT) { _ in
        NSLog("BharatTechKeepAwakeHelper: received SIGINT, restoring sleep")
        _ = allowSleep()
        if listenFd >= 0 { close(listenFd) }
        unlink(socketPath)
        exit(0)
...
@main
enum BharatTechKeepAwakeHelper {
    static func main() {
        installSignalHandlers()

        // Crash recovery: clear any stale SleepDisabled from a prior run.
        if isSleepDisabled() {
            NSLog("BharatTechKeepAwakeHelper: clearing stale SleepDisabled on startup")
            _ = allowSleep()
        }
        ...
        guard listenFd >= 0 else {
            NSLog("BharatTechKeepAwakeHelper: exiting — could not create listen socket")
            Foundation.exit(1)
        }

        NSLog("BharatTechKeepAwakeHelper: listening on \(socketPath)")

        // Accept loop — single-threaded, one connection at a time.
        // The BharatTech app sends short commands and reads the response immediately,
        // so blocking accept is fine.
        while true {
            let clientFd = accept(listenFd, nil, nil)
            guard clientFd >= 0 else {
                if errno == EINTR { continue }
                NSLog("BharatTechKeepAwakeHelper: accept failed: \(errno)")
                break
            }
