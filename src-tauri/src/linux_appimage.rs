...
const APP_ID: &str = "com.bharattech.app";
const APP_NAME: &str = "BharatTech";
const APP_VERSION: &str = env!("CARGO_PKG_VERSION");
const DESKTOP_FILE_NAME: &str = "com.bharattech.app.desktop";
const DESKTOP_ENTRY_MARKER: &str = "# Managed by BharatTech AppImage integration";
#[cfg(target_os = "linux")]
...
fn is_managed_entry(path: &Path) -> anyhow::Result<bool> {
    let raw = fs::read_to_string(path)
        .with_context(|| format!("failed to read desktop entry {}", path.display()))?;
    Ok(raw.contains(DESKTOP_ENTRY_MARKER) || raw.contains("X-BharatTech-Managed=true"))
}
...
mod tests {
    ...
    fn skips_when_disabled() {
        let paths = TestPaths::new();
        let mut env = integration_env(&paths, Some(paths.appimage_path("BharatTech.AppImage")));
        env.disabled = true;
    ...
    fn writes_managed_desktop_entry_and_icons() {
        let paths = TestPaths::new();
        let appimage_path = paths.appimage_path("BharatTech Beta.AppImage");
        ...
        assert!(desktop_entry.contains("X-BharatTech-Managed=true"));
    ...
    fn second_run_is_unchanged() {
        let paths = TestPaths::new();
        let env = integration_env(&paths, Some(paths.appimage_path("BharatTech.AppImage")));
    ...
    fn rewrites_desktop_entry_when_appimage_path_changes() {
        let paths = TestPaths::new();
        let first_env = integration_env(&paths, Some(paths.appimage_path("BharatTech One.AppImage")));
        let second_path = paths.appimage_path("BharatTech Two.AppImage");
        ...
        assert!(!desktop_entry.contains("BharatTech One.AppImage\" %U\nTryExec=BharatTech One.AppImage"));
    }
    ...
    fn preserves_unmanaged_local_entry_and_icons() {
        ...
        fs::write(
            ...
            "[Desktop Entry]\nName=BharatTech Custom\nIcon=custom-bharattech\n",
        )
        ...
        assert!(desktop_entry.contains("Name=BharatTech Custom"));
        assert!(!desktop_entry.contains(DESKTOP_ENTRY_MARKER));
        for size in [32_u32, 64, 128, 256, 512] {
            assert!(
                paths.icon_path(size).exists(),
    ...
    fn escapes_exec_path_with_spaces_and_percent_signs() {
        let paths = TestPaths::new();
        let appimage_path = paths.appimage_path("BharatTech 100%.AppImage");
    ...
    fn ignores_broken_system_desktop_entry() {
        ...
        fs::write(
            &system_entry,
            "[Desktop Entry]\nName=BharatTech\nExec=/definitely/missing/bharattech %U\n",
        )
